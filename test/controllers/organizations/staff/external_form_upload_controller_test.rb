  ...
  module Staff
    class ExternalFormUploadControllerTest < ActionDispatch::IntegrationTest
      setup do
        ...
        @params = {files: [file]}
        user = create(:admin)
        @person1 = create(:adopter, email: "adopter1@alta.com").person
        @person2 = create(:adopter, email: "willnotgetnewanswers@alta.com").person
        sign_in user
      end
      ...
      test "Creates form answers for person1 but not for person2" do
        assert_changes -> { @person1.form_answers.count } do
          assert_no_changes -> { @person2.form_answers.count } do
            post staff_external_form_upload_index_path, params: @params
          end
        end
      end
