...
class Organizations::FormPolicyTest < ActiveSupport::TestCase
  include PetRescue::PolicyAssertions

  context "context only action" do
    setup do
      @organization = ActsAsTenant.current_tenant
      @policy = -> { Organizations::FormPolicy.new(Form, user: @user, organization: @organization) }
    end
  ...
  context "existing record action" do
    setup do
      @form = create(:form)
      @policy = -> { Organizations::FormPolicy.new(@form, user: @user) }
    end
