...
* It is not necessary to generate coverage metrics every time the test suite is run. Therefore I have scoped its initiation under an environment variable. 
* After some experimentation, I found that it was not reporting accurate test coverage metrics when running tests in parallel workers. Therefore, when running the test suite with coverage, the number of workers should be set to one.
